...
impl<'a> Chat<'a> {
    ...
    pub async fn stream(
        ...
        if !status.is_success() {
            return Err(GroqError::from_response(response).await);
        }
...
impl ResponseFormat {
    ...
    pub fn json_schema(schema: serde_json::Value) -> ResponseFormatWithSchema {
...
#[derive(Debug, Clone, serde::Serialize, serde::Deserialize)]
pub struct ResponseFormatWithSchema {
    /// The type of response format.
    #[serde(rename = "type")]
    pub format_type: String,
    /// The JSON schema.
