...
use together_ai::{
    embeddings::{cosine_similarity, EmbeddingRequest},
    TogetherClient,
};
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for (i, embedding) in response.data.iter().enumerate() {
        println!("  Embedding {}: {} dimensions", i + 1, embedding.embedding.len());
    }
    ...
    if embeddings.len() >= 2 {
        ...
        println!("  'Machine learning...' <-> 'Deep learning...': {:.4}", sim_0_1);
        
        let sim_0_2 = cosine_similarity(embeddings[0], embeddings[2]);
        println!("  'Machine learning...' <-> 'The weather...': {:.4}", sim_0_2);
