//! Legacy completions API for vLLM.
...
impl<'a> Completions<'a> {
    ...
    ///
    /// ```no_run
    /// use vllm::{VllmClient, CompletionRequest};
    ///
    /// # async fn example() -> Result<(), Box<dyn std::error::Error>> {
    /// let client = VllmClient::new("http://localhost:8000")?;
    ///
    /// let request = CompletionRequest::builder("meta-llama/Llama-3-8b", "Once upon a time")
    ///     .max_tokens(100)
    ///     .build();
    ///
    ...
    ///     .build();
    ///
    /// let mut stream = client.completions().complete_stream(request).await?;
