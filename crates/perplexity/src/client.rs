...
impl PerplexityClient {
    ...
    pub fn with_config(config: ClientConfig) -> Result<Self> {
        ...
        headers.insert(
            AUTHORIZATION,
            HeaderValue::from_str(&auth_header).map_err(|_| PerplexityError::Config {
        ...
        let http = reqwest::Client::builder()
            ...
            .build()
            .map_err(|e| PerplexityError::Config {
        ...
        F: Fn() -> std::pin::Pin<Box<dyn std::future::Future<Output = Result<T>> + Send>>,
    {
        ...
        Err(PerplexityError::RetryExhausted {
            attempts,
    ...
    pub(crate) async fn handle_response(&self, response: reqwest::Response) -> Result<serde_json::Value> {
        ...
        if status.is_success() {
            ...
            Err(PerplexityError::from_response(response).await)
        }
    }
}
...
mod tests {
    ...
    fn test_client_creation() {
        let client = PerplexityClient::new("test-key").unwrap();
        assert_eq!(client.base_url(), DEFAULT_BASE_URL);
    }

    #[test]
    fn test_api_key_masked() {
        let client = PerplexityClient::new("sk-test1234567890").unwrap();
    ...
    fn test_api_key_masked_short() {
        let client = PerplexityClient::new("short").unwrap();
