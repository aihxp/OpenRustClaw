//! Fine-tuning API for Fireworks AI.
                ...
                Box::pin(async move {
                    let file_name =
                        file_path
                            .file_name()
                            .and_then(|n| n.to_str())
                            .ok_or_else(|| FireworksError::Config {
...
pub struct FineTuneRequest {
    ...
    /// Wether to use LoRA for fine-tuning.
    #[serde(skip_serializing_if = "Option::is_none")]
...
impl FineTuneRequest {
    ...
    pub fn lora(mut self, enabled: bool) -> Self {
        self.lora = Some(enabled);
        self
    }
}
