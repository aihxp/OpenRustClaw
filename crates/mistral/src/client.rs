...
pub mod endpoints {
    ...
    pub const AGENTS_COMPLETIONS: &str = "/agents/completions";
    /// FIM (Fill In the Middle) completions endpoint.
    pub const FIM_COMPLETIONS: &str = "/fim/completions";
}
...
/// A client for the Mistral AI API.
#[derive(Debug, Clone)]
...
impl MistralClient {
    ...
    pub fn embeddings(&self) -> crate::embedding::Embeddings<'_> {
        crate::embedding::Embeddings::new(self)
    }

    /// Get the agents API client (requires `agents` feature).
    #[cfg(feature = "agents")]
    pub fn agents(&self) -> crate::chat::Agents<'_> {
        crate::chat::Agents::new(self)
    }
        ...
        F: Fn() -> std::pin::Pin<Box<dyn std::future::Future<Output = Result<T>> + Send>>,
    {
        ...
        Err(MistralError::RetryExhausted {
            attempts,
        ...
        if status.is_success() {
            let body =
                response
                    ...
                    .await
                    .map_err(|e| MistralError::Internal {
                        message: format!("Failed to parse JSON response: {e}"),
                    })?;
            Ok(body)
        } else if status == reqwest::StatusCode::NOT_FOUND {
            let body = response.text().await.unwrap_or_default();
            Err(MistralError::NotFound {
                resource: "resource".to_string(),
                id: body,
            })
        } else {
            Err(MistralError::from_response(response).await)
        }
    }
}
...
mod tests {
    ...
    fn test_client_creation() {
        let client = MistralClient::new("test-key").unwrap();
