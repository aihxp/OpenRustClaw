...
| Signal | ⚠️ Partial | `signal-cli` daemon-backed direct/group send-receive, allowlist handling, registration/verify/link helpers, normalized route metadata, attachment/file-reference capture, normalized attachment ids/names/mime types, flat quote metadata, normalized group-member and mention metadata, receipt lifecycle events, and local agent/session routing are on the shipped runtime path |
