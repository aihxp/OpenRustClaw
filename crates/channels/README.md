...
| Telegram | ✅ Ready | Auth probe, outbound send, Bot API polling plus webhook ingress, native local-file uploads for media sends, reply threading, forum-topic metadata/topic targeting plus topic admin actions, poll handling, reactions, normalized inbound media/file-reference metadata, webhook registration/cleanup in webhook mode, and local agent/session routing |
...
| Slack | ✅ Ready | Auth probe, outbound send, edit/reaction operations, built-in Events API ingress, Socket Mode websocket ingress with envelope ACKs, `events_api` plus slash-command and interactive payload routing, thread ownership plus stream-mode metadata, draft-stream replies, native local-file uploads through Slack's external upload flow, attachment/file-reference metadata, download-action blocks, and local agent/session routing |
...
| Google Chat | ✅ Ready | Direct webhook ingress, Pub/Sub push-envelope decoding, token-backed or service-account outbound auth, response-mode gating, normalized space/user IDs, slash-command plus mention metadata capture, message body/thread/argument/slash/mention presence metadata, user-email and space-display-name presence flags, card-click and space lifecycle routing, structured attachment metadata capture with attachment-name/type/data-ref presence flags and file-reference counts, optional local attachment downloads with normalized `local_path` enrichment, first-class CLI operator flows for `status|connect|send|send-card|send-link-card`, and local agent/session routing are on the shipped runtime path |
...
| WhatsApp | ✅ Ready | Baileys bridge pairing/QR, inspectable QR/pairing state, first-class CLI operator flows for `status|connect|pair|send|send-media`, group/DM routing, mentions, replies, media send/receive, delivery acknowledgements, and local agent/session routing |
| Gmail Pub/Sub | ✅ Ready | Gmail watch setup/stop, Pub/Sub or direct-notification webhook ingress, history fetch, message hydration, structured attachment/file-reference metadata, normalized recipient/label counts plus recipient domains, recipient/domain/label/header presence flags, file-reference counts, attachment total-size metadata, unread/received-at plus parsed header/thread/history/body metrics, direct sends, threaded sends, replies with local-file attachments, metadata-driven label/archive/delete/forward actions, first-class CLI operator flows for `status|connect|process-notification|send|reply|label|archive|delete|forward`, and mail-triggered local agent/session routing are on the shipped runtime path |
