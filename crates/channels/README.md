...
| Matrix | ⚠️ Partial | Access-token or password auth, `/sync` polling ingress, outbound room sends, reply/thread relations, inbound reaction, redaction, and membership events, room join/leave, joined-room inspection, file uploads, and local agent/session routing are on the shipped runtime path |
