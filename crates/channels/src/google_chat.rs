...
impl GoogleChatWebhookHandler {
    ...
    async fn handle_message_event(&self, event: ChatEvent) -> Result<Option<serde_json::Value>> {
        ...
        let mut metadata = serde_json::json!({
            ...
            "google_chat_has_space_id": !space_id.is_empty(),
            "google_chat_has_user_id": !user_id.is_empty(),
            "google_chat_has_user_display_name": !user.display_name.is_empty(),
            "google_chat_has_event_time": !event.event_time.is_empty(),
        });
        if let Some(email) = user.email.as_ref() {
            metadata["google_chat_user_email"] = serde_json::json!(email);
            metadata["google_chat_has_user_email"] = serde_json::json!(true);
        } else {
            metadata["google_chat_has_user_email"] = serde_json::json!(false);
        }
        if let Some(display_name) = event.space.display_name.as_ref() {
            ...
            metadata["google_chat_space_display_name_length"] =
                serde_json::json!(display_name.chars().count());
            metadata["google_chat_has_space_display_name"] = serde_json::json!(true);
        } else {
            metadata["google_chat_has_space_display_name"] = serde_json::json!(false);
        }
        metadata["google_chat_user_display_name_length"] =
        ...
        if let Some(argument_text) = message.argument_text.as_ref() {
            ...
            metadata["google_chat_argument_length"] = serde_json::json!(argument_text.len());
            metadata["google_chat_has_argument"] = serde_json::json!(true);
        } else {
            metadata["google_chat_has_argument"] = serde_json::json!(false);
        }
        metadata["google_chat_body_length"] = serde_json::json!(message.text.len());
        if let Some(slash_command) = message.slash_command.as_ref() {
            ...
            if let Some(command_name) = slash_command.command_name.as_ref() {
                ...
                metadata["google_chat_slash_command_name_length"] =
                    serde_json::json!(command_name.chars().count());
            }
            metadata["google_chat_has_slash_command"] = serde_json::json!(true);
        } else {
            metadata["google_chat_has_slash_command"] = serde_json::json!(false);
        }
        ...
        if !mentioned_users.is_empty() {
            ...
            metadata["google_chat_has_mentions"] = serde_json::json!(true);
        } else {
            metadata["google_chat_has_mentions"] = serde_json::json!(false);
        }
        ...
        if let Some(thread) = &message.thread {
            ...
            metadata["google_chat_has_thread"] = serde_json::json!(true);
        } else {
            metadata["google_chat_has_thread"] = serde_json::json!(false);
        }
        ...
        if !message.attachments.is_empty() {
            ...
            if !attachment_names.is_empty() {
                ...
                metadata["google_chat_attachment_name_count"] = serde_json::json!(
                    metadata["google_chat_attachment_names"]
                        .as_array()
                        .map(|items| items.len())
                        .unwrap_or(0)
                );
                metadata["google_chat_has_attachment_names"] = serde_json::json!(true);
            } else {
            ...
            if !attachment_content_types.is_empty() {
                ...
                metadata["google_chat_attachment_content_type_count"] = serde_json::json!(
                    metadata["google_chat_attachment_content_types"]
                        .as_array()
                        .map(|items| items.len())
                        .unwrap_or(0)
            ...
            let attachment_data_refs: Vec<_> = message
                ...
                .collect();
            if !attachment_data_refs.is_empty() {
                metadata["google_chat_attachment_data_refs"] =
                    serde_json::json!(attachment_data_refs);
                metadata["google_chat_has_attachment_data_refs"] = serde_json::json!(true);
            } else {
            ...
            if !file_refs.is_empty() {
                metadata["google_chat_file_reference_count"] = serde_json::json!(file_refs.len());
    ...
    async fn handle_space_event(
        ...
        if let Some(user) = event.user.as_ref() {
            ...
            metadata["google_chat_user_display_name_length"] =
                serde_json::json!(user.display_name.chars().count());
            metadata["google_chat_has_user_id"] = serde_json::json!(!user_id.is_empty());
            metadata["google_chat_has_user_display_name"] =
                serde_json::json!(!user.display_name.is_empty());
            if let Some(email) = user.email.as_ref() {
                metadata["google_chat_user_email"] = serde_json::json!(email);
                metadata["google_chat_has_user_email"] = serde_json::json!(true);
            } else {
            ...
            metadata["google_chat_has_user_id"] = serde_json::json!(false);
            metadata["google_chat_has_user_display_name"] = serde_json::json!(false);
            metadata["google_chat_has_user_email"] = serde_json::json!(false);
        }
...
mod tests {
    ...
    async fn test_handle_card_clicked_event_routes_incoming_message() {
        ...
        assert_eq!(incoming.metadata["google_chat_has_action_parameters"], true);
        assert_eq!(
        ...
        assert_eq!(incoming.metadata["google_chat_has_space_id"], true);
        assert_eq!(incoming.metadata["google_chat_has_user_id"], true);
        assert_eq!(incoming.metadata["google_chat_has_user_display_name"], true);
    ...
    async fn test_handle_message_event_normalizes_attachment_file_references() {
        ...
        assert_eq!(
            incoming.metadata["google_chat_attachment_data_refs"][0],
