...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for model in &models.results {
        println!(
            ...
            model.owner,
            model.name,
            model.description.as_ref().map(|s| &s[..s.len().min(50)])
        );
    }
    ...
    let model = client
        .models()
        .get("black-forest-labs", "flux-schnell")
        .await?;
    ...
    let request = PredictionRequest::new().input("prompt", "A red cat sitting on a blue mat");
    ...
    let completed = client
        ...
        .run(
            "black-forest-labs",
            "flux-schnell",
            PredictionRequest::new().input("prompt", "A dog"),
        )
        .await?;
