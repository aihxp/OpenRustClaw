...
pub async fn metrics(config_path: &str) -> Result<()> {
    let _config = load_config(config_path);
    let workspace_root = std::env::current_dir()?;
    let result = voice_runtime::voice_metrics(&workspace_root).await?;
    println!("{}", serde_json::to_string_pretty(&result)?);
    Ok(())
}

pub async fn session_metrics(config_path: &str, session_id: &str) -> Result<()> {
    let _config = load_config(config_path);
    let workspace_root = std::env::current_dir()?;
    let result = voice_runtime::voice_session_metrics(&workspace_root, session_id).await?;
    println!("{}", serde_json::to_string_pretty(&result)?);
    Ok(())
}
...
pub async fn artifacts(config_path: &str, session_id: &str) -> Result<()> {
    let _config = load_config(config_path);
    let workspace_root = std::env::current_dir()?;
    let result = voice_runtime::voice_session_artifacts(&workspace_root, session_id).await?;
    println!("{}", serde_json::to_string_pretty(&result)?);
    Ok(())
}

pub async fn events(config_path: &str, session_id: &str) -> Result<()> {
    let _config = load_config(config_path);
    let workspace_root = std::env::current_dir()?;
    let result = voice_runtime::voice_session_events(&workspace_root, session_id).await?;
    println!("{}", serde_json::to_string_pretty(&result)?);
    Ok(())
}
