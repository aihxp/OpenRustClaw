...
pub async fn audit() -> Result<()> {
    ...
    for (env_var, name) in api_keys {
        match std::env::var(env_var) {
            Ok(key) => {
                if key.len() < 20 {
                    println!("  \x1b[33m⚠ {}: Key seems too short ({} chars)\x1b[0m", name, key.len());
                    warnings += 1;
                } else {
    ...
    let event = AuditEvent::new(
        "security_audit",
        if issues_found > 0 { AuditSeverity::Error } else { AuditSeverity::Info }
    )
    .with_details(&format!("Found {} issues, {} warnings", issues_found, warnings));
    event.log();
