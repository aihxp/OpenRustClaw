//! Streaming response handling for the Azure OpenAI API.
...
/// An async stream handler that can be used to process streaming responses.
...
impl StreamingHandler {
    ...
    /// Process a chunk and return the content delta.
    pub fn process<'a>(&mut self, chunk: &'a ChatCompletionChunk) -> Option<&'a str> {
        self.collector.process_chunk(chunk);
        chunk.content()
    }
    ...
    pub fn into_collector(self) -> StreamCollector {
        self.collector
    }
}
...
mod tests {
    ...
    #[test]
    fn test_chunk_done() {
        let chunk = ChatCompletionChunk::done();
    ...
    fn test_streaming_handler() {
        ...
        let content = handler.process(&chunk);
        assert_eq!(content, Some("Test"));
        assert!(handler.collector().is_complete());
    }
}
