...
| Plugin agent tools | Plugin-provided tool surface exposed to agent runs | plugin | partial | `crates/skills`, `crates/mcp`, `crates/agent`, `crates/cli/src/commands/start.rs` | `crates/skills/src/loader.rs`, `crates/skills/src/compiler.rs`, `crates/skills/src/sandbox.rs`, `crates/cli/src/commands/start.rs` | `docs/src/guides/skills.md`, `docs/roadmap.md` | high | Skills/tool injection exist, compiled help/schema caches now emit MCP/CLI-ready artifacts, and the built-in MCP server now exposes live compiled-skill summary/detail/reference tools, but full executable plugin parity is not complete |
