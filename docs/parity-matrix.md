...
| Plugin agent tools | Plugin-provided tool surface exposed to agent runs | plugin | partial | `crates/skills`, `crates/mcp`, `crates/agent`, `crates/cli/src/commands/start.rs`, `crates/cli/src/commands/skills.rs` | `crates/skills/src/loader.rs`, `crates/skills/src/compiler.rs`, `crates/skills/src/extension.rs`, `crates/skills/src/sandbox.rs`, `crates/cli/src/commands/start.rs`, `crates/cli/src/commands/skills.rs` | `docs/src/guides/skills.md`, `docs/roadmap.md` | high | Skills/tool injection exist, compiled help/schema caches now emit MCP/CLI-ready artifacts, the built-in MCP server exposes live compiled-skill summary/detail/reference tools plus dynamic `skill.<name>.execute` tools for compiled `.wasm`/`.wat` components, the generated CLI invoke bridge now reuses the same compiled cache, a first-class `extension_manifest.json` now defines the Rust-native contract for WASI components/background services/command hooks/tool injection, and compiled `.wasm`/`.wat` components can now execute through the bounded Rust sandbox; full plugin parity for auth/voice/channel/background workflows is still not complete |
