...
| Voice-note transcription | Provider-backed audio transcription in message flows | core | partial | `crates/cli/src/commands/voice_runtime.rs`, `crates/cli/src/commands/start.rs`, `crates/core/src/config.rs`, `crates/voice` | `cargo test -p openrustclaw-cli voice_runtime`, `cargo test -p openrustclaw-cli inbound_channel_messages_reuse_session_for_same_route` | `README.md`, `docs/roadmap.md`, `docs/feature-matrix.md` | high | Provider-backed inbound voice-note transcription and transcript injection now exist for supported channel attachments that expose a local path or directly fetchable media URL, but full OpenClaw-style voice/media parity is not complete yet |
