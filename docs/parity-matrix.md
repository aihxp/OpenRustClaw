...
| Web Control UI | Browser dashboard for chat, config, sessions, and nodes | core | partial | `crates/cli/src/commands/start.rs`, `crates/cli/src/commands/control_ui.rs` | `cargo test -p openrustclaw-cli`, `mdbook build docs` | `docs/roadmap.md`, `docs/parity-positioning.md`, `docs/feature-matrix.md` | high | An initial `/control/ui` dashboard now exists for runtime/config/channels/orchestration/browser inspection, service status, scheduler health, enabled-channel readiness, recent runtime events, live runtime logs, sessions, memory, jobs, config validate/apply, vault key editing, autonomy-policy and decision-lesson inspection/curation, orchestration run supervision detail, reflection-candidate promotion, orchestration trace plus delegation-relationship inspection, orchestration resource summaries, and live diagnostics, but full operator parity for chat, nodes, and extensions is still open |
