...
| Plugin agent tools | Plugin-provided tool surface exposed to agent runs | plugin | partial | `crates/skills`, `crates/mcp`, `crates/agent` | `crates/skills/src/loader.rs`, `crates/skills/src/compiler.rs`, `crates/skills/src/sandbox.rs` | `docs/src/guides/skills.md`, `docs/roadmap.md` | high | Skills/tool injection exist and a compiled help/schema cache now emits MCP/CLI-ready artifacts, but full plugin parity is not complete |
