...
| Typed control/config/diagnostics API | real | The runtime now exposes shared typed HTTP/WS control surfaces for `/control/runtime`, `/control/config`, `/control/config/validate`, `/control/diagnostics`, `/control/diagnostics/ws`, `/control/services/status|scheduler|runtime-events`, and `/control/channels/...` so future UIs and external clients can reuse the same state model |
| Web Control UI | partial | A shipped `/control/ui` dashboard now reuses the typed control APIs for runtime/config/channel/orchestration/browser inspection, service status, scheduler health, recent runtime events, live diagnostics streaming, runtime reload, and bounded browser actions; full operator parity for chat, sessions, memory, jobs, logs, nodes, and extension management remains open |
