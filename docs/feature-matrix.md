...
| Observability / LangSmith tracing | partial | Sidecar workflow traces now preserve trace ids back to Rust, and both scheduler dispatch and channel message handling can emit Rust-side LangSmith runs when enabled via env; coverage is still not uniform across all runtime paths |
