...
| Teams | partial | Bot Framework channel is on the shipped startup path with webhook ingress, JWT verification, outbound sends, reply aliases, conversation/reaction lifecycle routing with normalized member id/count metadata, attachment metadata capture plus opt-in local downloads, adaptive-card file links, and local agent/session routing; deeper operator/media parity still trails the tier-1 channels |
