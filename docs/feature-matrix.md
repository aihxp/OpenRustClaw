...
| Control-plane registry | real | `.claw/control/` now provides agent profiles, model profiles, Claw manifests, runtime-mode/task/category bindings, MCP inspection tools, onboarding scaffolding, doctor validation, and shared `/control/runtime` API inspection |
| Typed control/config/diagnostics API | real | The runtime now exposes shared typed HTTP/WS control surfaces for `/control/runtime`, `/control/config`, `/control/config/validate`, `/control/diagnostics`, `/control/diagnostics/ws`, `/control/services/status|scheduler|runtime-events`, `/control/sessions...`, `/control/memory/...`, `/control/jobs...`, and `/control/channels/...` so future UIs and external clients can reuse the same state model |
| Web Control UI | partial | A shipped `/control/ui` dashboard now reuses the typed control APIs for runtime/config/channel/orchestration/browser inspection, service status, scheduler health, recent runtime events, session inspection, memory inspection, scheduled-job inspection, config validate/apply, vault key set/delete, live diagnostics streaming, runtime reload, and bounded browser actions; full operator parity for chat, logs, nodes, and extension management remains open |
...
| Multi-model orchestration / quarterback model | partial | Bounded `openrustclaw orchestrate resolve|run` plus `/control/orchestration/resolve|run` surfaces now resolve claws/model profiles from `.claw/control`, fall back across model profiles when a provider is unsupported or unavailable, execute direct or orchestrated multi-model runs, and persist orchestration receipts; full live orchestrated runtime remains open |
