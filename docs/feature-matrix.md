...
| Observability / LangSmith tracing | partial | Sidecar workflow traces now preserve trace ids back to Rust, and scheduler dispatch, channel message handling, gateway chat completions, plus internal memory/RAG endpoints can emit Rust-side LangSmith runs when enabled via env; coverage is still not uniform across all runtime paths |
...
| RAG pipeline | partial | Deterministic collection-backed retrieval, Rust-backed durable chunk storage, budgeted context assembly, source-type filters, stopword-aware lexical scoring, and configurable per-source diversity limits exist; richer indexing still needs production hardening |
...
| Discord | partial | Auth probe, outbound send, verified Interactions HTTP ingress, Gateway `MESSAGE_CREATE` receive, reconnect plus session resume handling, deferred acknowledgements, and local agent/session routing exist; deeper gateway polish remains incomplete |
