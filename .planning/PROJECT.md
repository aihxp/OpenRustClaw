...
- **Greenfield conversion baseline:** canonical `18/18` ranked seams migrated, or `100%` complete, across the retired historical transition inventory; the follow-on full-conversion roadmap is also closed at `6/6` shipped milestones, or `100%`; the native-delivery planning roadmap is also closed at `8/8` shipped milestones, or `100%`; the native-delivery implementation roadmap is also closed at `6/6`, or `100%`; the new native-product E2E roadmap now starts at `0/1`, or `0%`

## Most Recent Milestone: v1.38 Native Delivery Implementation: Native Product Verification and Packaging Exit

**Result:** Shipped 2026-03-28. OpenRustClaw closed the final source-level native-delivery implementation milestone by verifying the native-delivery scorecard against the shipped source tree, aligning packaging and canonical docs to the implemented crate and entrypoint story, auditing the remaining bounded compatibility exceptions explicitly, and defining the final native-product claim boundary against source evidence. The native-delivery implementation roadmap now stands at `6/6`, or `100%`.

**Archive:** `.planning/milestones/v1.38-ROADMAP.md`, `.planning/milestones/v1.38-REQUIREMENTS.md`, `.planning/milestones/v1.38-MILESTONE-AUDIT.md`, `.planning/milestones/v1.38-VERIFICATIONS.md`

## Current Milestone: v1.39 Native Product E2E Verification and Greenfield Repairs

**Goal:** Run full end-to-end product verification across the shipped native and compatibility paths, classify any failures by ownership, and apply any required repairs through greenfield-native surfaces instead of re-expanding legacy command ownership.

**Target features:**
- verify the real CLI, control, gateway, MCP, runtime, and operator paths end to end instead of relying on milestone memory
- classify failures with explicit ownership and evidence so repairs land in the right layer
- apply needed repairs through `openrustclaw-app`, native delivery crates, or explicit infrastructure adapters before touching legacy command surfaces
...
- ✓ Dedicated runtime-host and background-worker entrypoints are now defined over app ports instead of remaining implicit legacy command bootstraps — v1.29 Phase 121
...
- ✓ The first legacy-module retirement inventory now defines which command-tree hotspots leave the main product path first and what retirement state each one enters — v1.31 Phase 129
- ✓ Still-live legacy surfaces now have explicit compatibility-shim or delete boundaries instead of open-ended compatibility ownership — v1.31 Phase 130
- ✓ The `main.rs` bootstrap-retirement path is now defined explicitly enough to move top-level binary routing off the legacy command tree — v1.31 Phase 131
- ✓ Guardrails and verification rules now make retired delivery files provably unable to regain main product-path ownership — v1.31 Phase 132
- ✓ The native-delivery roadmap now defines the final scorecard for judging native product-path ownership explicitly instead of by milestone memory — v1.32 Phase 133
- ✓ Docs, packaging, and contributor-guidance alignment to the native architecture are now defined explicitly instead of being left as implied follow-on cleanup — v1.32 Phase 134
- ✓ Remaining compatibility shims and exceptions now require explicit classification, ownership, and exit criteria instead of being hidden inside a completion claim — v1.32 Phase 135
- ✓ The final native-product exit claim is now explicit about what the repo can and cannot claim truthfully on roadmap completion alone — v1.32 Phase 136
- ✓ The implementation roadmap now defines the first gateway-native control bootstrap slice explicitly enough to implement without rediscovering startup ownership — v1.33 Phase 137
- ✓ The implementation roadmap now defines the first MCP-native startup slice plus the Control UI serving alignment needed for the successor startup path — v1.33 Phase 138
- ✓ The implementation roadmap now defines the first bounded `start.rs` startup handoff instead of leaving bootstrap removal vague — v1.33 Phase 139
- ✓ The implementation roadmap now defines the direct successor-entry verification and compatibility rules for the initial gateway and MCP handoff slice — v1.33 Phase 140
- ✓ The implementation roadmap now defines the first native CLI dispatch bootstrap slice explicitly enough to implement without rediscovering top-level routing ownership — v1.34 Phase 141
- ✓ The implementation roadmap now defines the first assistant, session, and inspect native CLI operator-path slice over successor CLI ownership — v1.34 Phase 142
- ✓ The implementation roadmap now defines the first control and runtime CLI handoff slice instead of leaving those paths under broad legacy command ownership — v1.34 Phase 143
- ✓ The implementation roadmap now defines the direct compatibility and verification rules for the first native CLI successor path — v1.34 Phase 144
- ✓ The implementation roadmap now defines the first native runtime-host bootstrap slice for service-manager, probes, maintenance, and scheduler ownership — v1.35 Phase 145
- ✓ The implementation roadmap now defines the runtime startup-boundary contracts needed to move worker lifecycle startup off the legacy command layer — v1.35 Phase 146
- ✓ The implementation roadmap now defines the mobile, voice, and orchestration worker-boot migration slice over native runtime-host ownership — v1.35 Phase 147
- ✓ The implementation roadmap now defines the direct compatibility and verification rules for the first native runtime-host handoff slice — v1.35 Phase 148
- ✓ The implementation roadmap now defines the first repository-adapter inventory and successor ownership slice for sqlite, workspace files, audit logs, runtime config, compiled-skill cache, and registries — v1.36 Phase 149
- ✓ The implementation roadmap now defines the first integration gateway slice for providers, channels, and external service integrations over native infrastructure ownership — v1.36 Phase 150
- ✓ The implementation roadmap now defines app-port to repository-adapter alignment needed to move persistence-heavy and side-effect-heavy ownership off command-local helpers — v1.36 Phase 151
- ✓ The implementation roadmap now defines the direct verification and compatibility rules for the first repository and integration adapter lift slice — v1.36 Phase 152
- ✓ The implementation roadmap now defines the first legacy-module retirement inventory and successor ownership slice for superseded command-tree hotspots — v1.37 Phase 153
- ✓ The implementation roadmap now defines the first delete-or-shim boundaries plus `main.rs` bootstrap retirement path over native ownership — v1.37 Phase 154
- ✓ The implementation roadmap now defines the first guardrails needed to keep retired command-tree hotspots out of the main product path — v1.37 Phase 155
- ✓ The implementation roadmap now defines the direct verification and compatibility rules for the first legacy command-tree retirement slice — v1.37 Phase 156
- ✓ The native-delivery scorecard is now verified directly against the shipped source tree and implemented entrypoints instead of relying on roadmap memory alone — v1.38 Phase 157
- ✓ Packaging, canonical docs, and contributor guidance now align to the implemented native delivery surfaces and current crate layout instead of the retired command-tree story — v1.38 Phase 158
...
The next follow-on queue now turns that closed planning denominator into source-level implementation work. `v1.33` started the native-delivery implementation roadmap by defining the first concrete gateway, MCP, Control UI, and `start.rs` handoff slice needed for real successor entrypoint work.

The next implementation queue now extends that same posture into user-facing CLI ownership. `v1.34` started the native CLI dispatch and core operator-path slice so top-level CLI routing, assistant/session inspection flows, and the first control/runtime command paths can leave the legacy command tree through an explicit successor-entry implementation contract.

The next implementation queue then turned that same source-level posture toward runtime lifecycle startup itself. `v1.35` started the native runtime-host and background-worker slice so service-manager startup, probes, maintenance, scheduler flows, and the first mobile or voice or orchestration worker boot paths can leave the legacy command layer through an explicit successor-entry implementation contract.

The next implementation queue then turned that same source-level posture toward persistence and side-effect ownership. `v1.36` started the repository and integration adapter lift slice so sqlite-backed state, workspace-file ownership, audit and registry persistence, runtime config writes, and external provider or channel integrations can leave command-local helpers through explicit native infrastructure contracts.

The next implementation queue then turned that same source-level posture toward legacy-surface shutdown itself. `v1.37` defined the first legacy command-tree retirement inventory, delete-or-shim boundaries, `main.rs` bootstrap retirement path, and retirement guardrails so superseded command modules can leave the primary product path through explicit successor ownership instead of remaining indefinitely as compatibility debt.
