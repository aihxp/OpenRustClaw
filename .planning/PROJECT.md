...
- **Planning state:** active milestone `v1.31 Native Delivery Layer: Legacy Module Retirement and Compatibility Shutdown`
...
## Current Milestone: v1.31 Native Delivery Layer: Legacy Module Retirement and Compatibility Shutdown

**Goal:** Continue replacing the legacy delivery layer by defining the legacy-module retirement and compatibility-shutdown slice so superseded command modules can leave the main product path.
...
The next native-delivery queue now moves from replacement contracts into legacy retirement itself. `v1.31` targets the removal or isolation of superseded legacy modules, the bounded shutdown or shimming of still-live compatibility entrypoints, the final narrowing of `main.rs`, and the guardrails needed to prove the old command tree is no longer the main product path.
...
- the native-delivery roadmap now stands at `6/8`, or `75%`, with `v1.31` targeting `7/8`, or about `88%`
- contributor defaults and source-level tests still preserve `openrustclaw-app` as the default home for new business logic while `v1.31` focuses on legacy-module retirement and compatibility shutdown
...
---
*Last updated: 2026-03-28 after starting v1.31 milestone*
