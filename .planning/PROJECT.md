...
- **Greenfield conversion baseline:** canonical `18/18` ranked seams migrated, or `100%` complete, across the retired historical transition inventory; the follow-on full-conversion roadmap is also closed at `6/6` shipped milestones, or `100%`; the native-delivery planning roadmap is also closed at `8/8` shipped milestones, or `100%`; the native-delivery implementation roadmap now stands at `4/6` shipped milestones, or about `67%`, with `v1.37` targeting `5/6`, or about `83%`

## Most Recent Milestone: v1.36 Native Delivery Implementation: Repository and Integration Adapter Lift

**Result:** Shipped 2026-03-28. OpenRustClaw closed the fourth source-level native-delivery implementation milestone by defining the first repository-adapter inventory and successor ownership slice, the first integration gateway slice for providers, channels, and external service integrations, the first app-port to repository-adapter alignment slice, and the first direct repository-lift compatibility-and-verification rules. The native-delivery implementation roadmap now stands at `4/6` shipped milestones, or about `67%`.

**Archive:** `.planning/milestones/v1.36-ROADMAP.md`, `.planning/milestones/v1.36-REQUIREMENTS.md`, `.planning/milestones/v1.36-MILESTONE-AUDIT.md`, `.planning/milestones/v1.36-VERIFICATIONS.md`

## Current Milestone: v1.37 Native Delivery Implementation: Legacy Command Tree Retirement

**Goal:** Continue the native-delivery implementation roadmap from `4/6` shipped milestones, or about `67%`, by defining the first real legacy command-tree retirement slice needed to move superseded command modules and `main.rs` out of the primary product path.

**Target features:**
- define the first legacy module retirement inventory and successor ownership slice for superseded command-tree hotspots
- define the first delete-or-shim boundaries plus `main.rs` bootstrap retirement path over native ownership
- define the first retirement guardrails plus direct verification rules while preserving the closed denominators
...
The next implementation queue now turns that same source-level posture toward legacy-surface shutdown itself. `v1.37` starts the legacy command-tree retirement slice so superseded command modules and `main.rs` can leave the primary product path through explicit successor ownership, delete-or-shim boundaries, and retirement guardrails.
...
- the native-delivery implementation roadmap now stands at `4/6`, or about `67%`, with `v1.37` targeting `5/6`, or about `83%`
...
---
*Last updated: 2026-03-28 after starting v1.37 milestone*
