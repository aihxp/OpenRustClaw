...
## Shipped Release Ledger

| Release | Shipped baseline captured here |
|---------|--------------------------------|
| `1.4.1` | Provider-specific onboarding access modes, verified bootstrap evidence, explicit primary task model selection, and setup-handoff continuity |
| `1.4.2` | Delegated local-agent onboarding lanes for Claude Code, Codex, and Gemini CLI plus runtime model validation |
| `1.4.3` | Provider-family-first onboarding, local-agent-aware defaults, and real WhatsApp pairing bootstrap |
| `1.4.4` | Fresh-workspace config scaffolding, workspace-root config resolution, and less noisy Ollama repair checks |
| `1.4.5` | Tailscale tailnet onboarding guidance, persisted gateway network mode, narrower delegated CLI discovery, and faster runtime listener conflict failure |
| `1.4.6` | Truthful doctor and repair continuity for delegated or local-runtime lanes, safer delegated runtime switching, and cleaner doctor output |
| `1.4.7` | Tailscale-first advertised gateway access plus correct managed restart and runtime beacon listener reporting |
| `1.4.8` | Repo hygiene for local operator and runtime artifacts so normal usage does not pollute the worktree |
| `1.4.9` | First-class `start` or `stop` or `restart` runtime control, startup preflight update warnings, safe OpenClaw migration, and repo-local temp-dir defaults for heavy release workflows |
...
- [x] **Phase 196: Ad Hoc Release Audit and Catch-Up Ledger** - Inventory the shipped `1.4.1` through `1.4.9` changes and record them as one truthful milestone baseline. (completed 2026-04-09)
- [x] **Phase 197: Onboarding, Runtime, and Operator Journey Convergence** - Align the active planning story with the shipped onboarding, repair, delegated-agent, Tailscale, and runtime-control behavior. (completed 2026-04-09)
- [x] **Phase 198: Release and Operational Hardening Capture** - Preserve the shipped release-lane, update-check, OpenClaw migration, temp-dir mitigation, and runtime lifecycle hardening as canonical operational baseline. (completed 2026-04-09)
- [x] **Phase 199: GSD Re-entry Closeout and Next Queue Definition** - Finish the catch-up milestone with an explicit next phase so future work proceeds through GSD instead of ad hoc release flow. (completed 2026-04-09)
...
|-------|----------------|--------|-----------|
| 196. Ad Hoc Release Audit and Catch-Up Ledger | 1/1 | Complete    | 2026-04-09 |
| 197. Onboarding, Runtime, and Operator Journey Convergence | 1/1 | Complete    | 2026-04-09 |
| 198. Release and Operational Hardening Capture | 1/1 | Complete    | 2026-04-09 |
| 199. GSD Re-entry Closeout and Next Queue Definition | 1/1 | Complete    | 2026-04-09 |
...
- **Name:** `Release Traceability and Milestone Correlation`
