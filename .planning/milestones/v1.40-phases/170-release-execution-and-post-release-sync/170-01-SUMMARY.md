...
The `1.4.0` public release is shipped. `openrustclaw-core 1.4.0` is now published on crates.io, the new crates.io token was saved locally with `cargo login`, the repo release-alignment commit landed, and the public tag `v1.4.0` was pushed so the repo’s version line matches the shipped crate.
