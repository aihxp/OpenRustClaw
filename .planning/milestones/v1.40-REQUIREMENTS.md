...
**Public-product convergence roadmap baseline after shipment:** `1/1`, or `100%`

## Scope

This milestone cleaned and simplified the repo without breaking the shipped product, converged public docs and metadata to the product’s current public story, repaired the checked-in GitHub Actions and release automation surfaces, and shipped the next public release including crates.io publication.

## Milestone Requirements

- [x] **PPC-01**: Public-facing docs, package metadata, and repo entrypoints no longer rely on internal migration terminology and instead describe the shipped product plainly.
- [x] **PPC-02**: Stale or duplicated code, docs, workflows, and support files are deleted, merged, or simplified only where verification shows the cleanup is safe.
- [x] **PPC-03**: Relevant GitHub Actions and release automation workflows are green, or intentionally retired with explicit rationale.
- [x] **PPC-04**: The next public release is prepared and shipped with synchronized versions, release notes, package metadata, and crates.io publication state.
