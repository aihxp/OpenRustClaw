...
- Most recent shipment: **v1.24 Full Greenfield Conversion: Adapter-Only Exit and Enforcement**
...
- Current execution: **full-conversion program complete**
- Next step: `$gsd-new-milestone`

## Program Status

There is no active milestone. The six-milestone full-conversion roadmap is complete and archived through `v1.24`.

### Current Queue Rule

The original ranked greenfield seam inventory remains closed at `18/18` and retired. The follow-on adapter-only roadmap in `.planning/codebase/GREENFIELD-FULL-CONVERSION.md` is now also complete at `6/6`. Any deeper architecture work requires a new canonical queue instead of silently extending either finished denominator.
