...
The remaining closeout order is deliberate: operator-trust surfaces, the trust-first orchestration runtime, and the browser/tool-group slice are now the shipped baseline, so the next closeout work is extension parity plus broader runtime resilience, followed by the remaining voice/media/mobile gaps and the final parity audit.
