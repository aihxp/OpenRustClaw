...
- onboarding now detects existing workspace state, offers keep/modify/reset-with-backup choices, supports QuickStart vs Advanced paths, finishes with a `doctor`-backed health handoff, and only offers to launch the persisted assistant once provider and workspace readiness pass the first-start gate
