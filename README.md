...
Inbound voice-note transcription can be enabled on the runtime path for supported channel attachments that already expose a local file path or a directly fetchable media URL, and operators now also have explicit `openrustclaw voice status|providers|sessions|session-health|prewarm|reap-sessions|voices|transcribe|synthesize|start-session|session-status|append-user|respond|reconnect-session|end-session` plus matching `/control/voice/...` surfaces for readiness inspection, provider catalog/status across OpenAI-compatible STT/TTS plus Deepgram STT, bounded persisted voice-session receipts, session health/reap/prewarm/reconnect controls, ad hoc transcription, TTS voice discovery, and synthesized audio artifacts. A bounded media operator lane also exists through `openrustclaw media providers|inspect|extract-text` and `/control/media/providers|inspect|extract-text` for media-provider readiness inspection, local image/document inspection, OCR-backed image extraction, bounded local rich-document extraction for `docx` and `rtf`, optional local `pdf` extraction when `pdftotext` is present, and bounded local-audio text extraction over the shipped STT lanes where the runtime already has real local/provider primitives.
