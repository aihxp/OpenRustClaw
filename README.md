...
For the operator trust loop, start with `openrustclaw runtime services channels` or `/control/services/channels`, then review `Recent Email Activity` and `Recent Voice Outcomes` in `/control/ui` before drilling into raw `/control/voice/...` session detail or execution-ledger entries. Gmail ingress receipts are retained as `channels.gmail_pubsub.ingress` records in `.claw/control/tool-executions.jsonl`, while the voice outcome summaries are derived from persisted session receipts under `.claw/voice/sessions/`.
