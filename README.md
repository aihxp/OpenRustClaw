...
The remaining closeout order is deliberate: operator-trust surfaces, the trust-first orchestration runtime, the browser/tool-group slice, extension operator management, and the broader runtime reload/resilience lane are now the shipped baseline. Wave 4 is now partially underway with provider-backed inbound voice-note transcription and transcript injection for supported attachment refs, voice CLI/API/UI status plus catalog/transcription/synthesis surfaces, and file-backed mobile node registry plus preview/operator surfaces, but real device-command parity and the final parity audit still remain. The later long-term Rust-native plugin/channel model still remains a distinct roadmap track.
