...
The remaining closeout order is deliberate: operator-trust surfaces, the trust-first orchestration runtime, the browser/tool-group slice, extension operator management, and the broader runtime reload/resilience lane are now the shipped baseline, so the next closeout work is Wave 4: the remaining voice/media/mobile gaps plus the final parity audit. The later long-term Rust-native plugin/channel model still remains a distinct roadmap track.
