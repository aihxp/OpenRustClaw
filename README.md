...
- Google Chat: webhook ingress, token-backed or service-account outbound auth, response-mode gating, slash-command metadata capture, card/file-reference responses, attachment metadata capture, and local agent/session routing are implemented; richer operator/media parity still remains
