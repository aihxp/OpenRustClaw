...
- Teams: Bot Framework webhook ingress, JWT verification, outbound sends, reply aliases, conversation/reaction lifecycle routing with normalized member id/count metadata, attachment metadata capture plus opt-in local downloads, adaptive-card file links, and local agent/session routing are on the shipped runtime path, but it is still partial relative to the tier-1 surfaces
