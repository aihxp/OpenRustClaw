...
Inbound voice-note transcription can be enabled on the runtime path for supported channel attachments that already expose a local file path or a directly fetchable media URL, and operators now also have explicit `openrustclaw voice status|providers|voices|transcribe|synthesize` plus `/control/voice/status|providers|voices|transcribe|synthesize` surfaces for readiness inspection, OpenAI-compatible provider catalog/status, ad hoc transcription, TTS voice discovery, and synthesized audio artifacts.
