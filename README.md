...
- Google Chat: direct webhook ingress plus Pub/Sub push-envelope decoding, token-backed or service-account outbound auth, response-mode gating, normalized space/user IDs, slash-command metadata plus mention capture, message ID plus body/thread/argument/slash/mention presence metadata, user-email and space-display-name presence flags, card-click and space lifecycle routing, action-name/invoked-function/form-input/action-parameter presence metadata, event-time/user-email metadata, card/file-reference responses, structured attachment/file-reference metadata plus attachment-name/type/data-ref presence capture and file-reference counts, and local agent/session routing are implemented; richer operator/media parity still remains
