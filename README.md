...
Inbound voice-note transcription can be enabled on the runtime path for supported channel attachments that already expose a local file path or a directly fetchable media URL, and operators now also have explicit `openrustclaw voice status|voices|transcribe|synthesize` plus `/control/voice/status|voices|transcribe|synthesize` surfaces for inspection, ad hoc transcription, TTS voice discovery, and synthesized audio artifacts.
