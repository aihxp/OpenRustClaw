...
- Signal: `signal-cli` daemon-backed direct/group send-receive, allowlist handling, registration/verify/link helpers, normalized route metadata, outbound local-attachment sends, attachment/file-reference capture, mention-aware group routing, and local agent/session routing are implemented; richer operator UX and inbound media download parity still remain
