...
- Google Chat: direct webhook ingress plus Pub/Sub push-envelope decoding, token-backed or service-account outbound auth, response-mode gating, slash-command metadata plus mention capture, card-click and space lifecycle routing, card action parameter metadata, event-time/user-email metadata, card/file-reference responses, structured attachment/file-reference metadata capture, and local agent/session routing are implemented; richer operator/media parity still remains
